...
Plain retrieval-augmented translation only *suggests* glossary terms to
the LLM inside the prompt -- there's no guarantee the model actually uses
them. This module turns glossary usage into a verified, enforced step
instead of a hope:

1. DETECT -- scan the *source* English text for exact glossary terms
             (phrase-level entries checked before word-level ones, so
             "personal data" is required as a single unit rather than
...
required, terms satisfied, retry count) -- useful evidence that the
glossary is doing real work rather than being retrieved and ignored.
"""
...
def _word_boundary_pattern(term: str) -> re.Pattern:
    """Matches `term` only as a whole word/phrase, not as part of a
    larger word (so 'data' doesn't match inside 'database')."""
    return re.compile(r"(?<!\w)" + re.escape(term) + r"(?!\w)", re.IGNORECASE)


def detect_required_terms(source_text: str) -> List[VocabEntry]:
    """Finds every glossary term present in the source text. Phrase-level
    entries are checked before word-level ones (longest term first), and
    once a term is matched its span is masked out of the working text so
    a shorter word contained within an already-matched phrase (e.g.
    'data' inside 'personal data') isn't separately flagged too."""
    indices = get_indices()
    ...
    for entry in candidates:
        ...
        if match:
            ...
            # Blank out the matched span so it can't also trigger a
            # shorter, subsumed term match later in the loop.
            working_text = (
...
def build_enforcement_block(required_terms: List[VocabEntry]) -> str:
    """Builds the prompt section listing non-negotiable required terms."""
...
def build_retry_correction_block(missing_terms: List[VocabEntry]) -> str:
    """Builds the corrective prompt addition used on retry, naming exactly
    which required terms were missed the first time."""
    lines = "\n".join(
